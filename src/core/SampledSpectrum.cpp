...
namespace idragnev::pbrt {
    ...
    SampledSpectrum::fromSortedSamples(SpectrumSamples spectrumSamples) {
        ...
        for (std::size_t i = 0; i < samples.size(); ++i) {
            ...
            samples[i] = averageSpectrumSamples(spectrumSamples.lambdas,
                                                spectrumSamples.values,
                                                WavelengthRegion{
                                                    .lambdaStart = lambda0,
                                                    .lambdaEnd = lambda1,
                                                });
    ...
    static constexpr std::size_t CIE_SAMPLES_COUNT = 471;

    static constexpr Float CIE_X[CIE_SAMPLES_COUNT] = {
    ...
    static constexpr Float CIE_Y[CIE_SAMPLES_COUNT] = {
    ...
    static constexpr Float CIE_Z[CIE_SAMPLES_COUNT] = {
    ...
    static constexpr Float CIE_LAMBDA[CIE_SAMPLES_COUNT] = {
    ...
    static constexpr SampledSpectrum X = SampledSpectrum::fromSortedSamples({
        .lambdas = std::span<const Float>(CIE_LAMBDA, CIE_SAMPLES_COUNT),
        .values = std::span<const Float>(CIE_X, CIE_SAMPLES_COUNT),
    });
    
    static constexpr SampledSpectrum Y = SampledSpectrum::fromSortedSamples({
        .lambdas = std::span<const Float>(CIE_LAMBDA, CIE_SAMPLES_COUNT),
        .values = std::span<const Float>(CIE_Y, CIE_SAMPLES_COUNT),
    });

    static constexpr SampledSpectrum Z = SampledSpectrum::fromSortedSamples({
        .lambdas = std::span<const Float>(CIE_LAMBDA, CIE_SAMPLES_COUNT),
        .values = std::span<const Float>(CIE_Z, CIE_SAMPLES_COUNT),
    });

    std::array<Float, 3> SampledSpectrum::toXYZ() const {
        std::array<Float, 3> xyz = {0.f, 0.f, 0.f};

        for (std::size_t i = 0; i < SampledSpectrum::NumberOfSamples; ++i) {
            xyz[0] += X[i] * (*this)[i];
            xyz[1] += Y[i] * (*this)[i];
            xyz[2] += Z[i] * (*this)[i];
        }
        ...
        const Float scale =
            static_cast<Float>(SAMPLED_WAVELENGTH_REGION.lambdaEnd -
                               SAMPLED_WAVELENGTH_REGION.lambdaStart) /
            static_cast<Float>(CIE_Y_INTEGRAL *
                               SampledSpectrum::NumberOfSamples);
        xyz[0] *= scale;
        xyz[1] *= scale;
        xyz[2] *= scale;
    ...
    static constexpr std::size_t RGB_TO_SPECT_SAPMLES_COUNT = 32;
...
#endif
    // clang-format off
    static constexpr Float RGB_TO_SPECT_LAMBDAS[RGB_TO_SPECT_SAPMLES_COUNT] = {
    ...
    static constexpr Float RGB_REFL_TO_SPECT_WHITE_VALUES[RGB_TO_SPECT_SAPMLES_COUNT] = {
    ...
    static constexpr Float RGB_REFL_TO_SPECT_CYAN_VALUES[RGB_TO_SPECT_SAPMLES_COUNT] = {
    ...
    static constexpr Float RGB_REFL_TO_SPECT_MAGENTA_VALUES[RGB_TO_SPECT_SAPMLES_COUNT] = {
    ...
    static constexpr Float RGB_REFL_TO_SPECT_YELLOW_VALUES[RGB_TO_SPECT_SAPMLES_COUNT] = {
    ...
    static constexpr Float RGB_REFL_TO_SPECT_RED_VALUES[RGB_TO_SPECT_SAPMLES_COUNT] = {
    ...
    static constexpr Float RGB_REFL_TO_SPECT_GREEN_VALUES[RGB_TO_SPECT_SAPMLES_COUNT] = {
    ...
    static constexpr Float RGB_REFL_TO_SPECT_BLUE_VALUES[RGB_TO_SPECT_SAPMLES_COUNT] = {
    ...
    static constexpr Float RGB_ILLUM_TO_SPECT_WHITE_VALUES[RGB_TO_SPECT_SAPMLES_COUNT] = {
    ...
    static constexpr Float RGB_ILLUM_TO_SPECT_CYAN_VALUES[RGB_TO_SPECT_SAPMLES_COUNT] = {
    ...
    static constexpr Float RGB_ILLUM_TO_SPECT_MAGENTA_VALUES[RGB_TO_SPECT_SAPMLES_COUNT] = {
    ...
    static constexpr Float RGB_ILLUM_TO_SPECT_YELLOW_VALUES[RGB_TO_SPECT_SAPMLES_COUNT] = {
    ...
    static constexpr Float RGB_ILLUM_TO_SPECT_RED_VALUES[RGB_TO_SPECT_SAPMLES_COUNT] = {
    ...
    static constexpr Float RGB_ILLUM_TO_SPECT_GREEN_VALUES[RGB_TO_SPECT_SAPMLES_COUNT] = {
    ...
    static constexpr Float RGB_ILLUM_TO_SPECT_BLUE_VALUES[RGB_TO_SPECT_SAPMLES_COUNT] = {
    ...
    static constexpr SampledSpectrum RGB_REFL_TO_SPECT_WHITE =
        SampledSpectrum::fromSortedSamples({
            .lambdas = std::span<const Float>(RGB_TO_SPECT_LAMBDAS,
                                              RGB_TO_SPECT_SAPMLES_COUNT),
            .values = std::span<const Float>(RGB_REFL_TO_SPECT_WHITE_VALUES,
                                             RGB_TO_SPECT_SAPMLES_COUNT),
        });

    static constexpr SampledSpectrum RGB_REFL_TO_SPECT_CYAN =
        SampledSpectrum::fromSortedSamples({
            .lambdas = std::span<const Float>(RGB_TO_SPECT_LAMBDAS,
                                              RGB_TO_SPECT_SAPMLES_COUNT),
            .values = std::span<const Float>(RGB_REFL_TO_SPECT_CYAN_VALUES,
                                             RGB_TO_SPECT_SAPMLES_COUNT),
        });

    static constexpr SampledSpectrum RGB_REFL_TO_SPECT_MAGENTA =
        SampledSpectrum::fromSortedSamples({
            .lambdas = std::span<const Float>(RGB_TO_SPECT_LAMBDAS,
                                              RGB_TO_SPECT_SAPMLES_COUNT),
            .values = std::span<const Float>(RGB_REFL_TO_SPECT_MAGENTA_VALUES,
                                             RGB_TO_SPECT_SAPMLES_COUNT),
        });

    static constexpr SampledSpectrum RGB_REFL_TO_SPECT_YELLOW =
        SampledSpectrum::fromSortedSamples({
            .lambdas = std::span<const Float>(RGB_TO_SPECT_LAMBDAS,
                                              RGB_TO_SPECT_SAPMLES_COUNT),
            .values = std::span<const Float>(RGB_REFL_TO_SPECT_YELLOW_VALUES,
                                             RGB_TO_SPECT_SAPMLES_COUNT),
        });

    static constexpr SampledSpectrum RGB_REFL_TO_SPECT_RED =
        SampledSpectrum::fromSortedSamples({
            .lambdas = std::span<const Float>(RGB_TO_SPECT_LAMBDAS,
                                              RGB_TO_SPECT_SAPMLES_COUNT),
            .values = std::span<const Float>(RGB_REFL_TO_SPECT_RED_VALUES,
                                             RGB_TO_SPECT_SAPMLES_COUNT),
        });

    static constexpr SampledSpectrum RGB_REFL_TO_SPECT_GREEN =
        SampledSpectrum::fromSortedSamples({
            .lambdas = std::span<const Float>(RGB_TO_SPECT_LAMBDAS,
                                              RGB_TO_SPECT_SAPMLES_COUNT),
            .values = std::span<const Float>(RGB_REFL_TO_SPECT_GREEN_VALUES,
                                             RGB_TO_SPECT_SAPMLES_COUNT),
        });

    static constexpr SampledSpectrum RGB_REFL_TO_SPECT_BLUE =
        SampledSpectrum::fromSortedSamples({
            .lambdas = std::span<const Float>(RGB_TO_SPECT_LAMBDAS,
                                              RGB_TO_SPECT_SAPMLES_COUNT),
            .values = std::span<const Float>(RGB_REFL_TO_SPECT_BLUE_VALUES,
                                             RGB_TO_SPECT_SAPMLES_COUNT),
        });

    static constexpr SampledSpectrum RGB_ILLUM_TO_SPECT_WHITE =
        SampledSpectrum::fromSortedSamples({
            .lambdas = std::span<const Float>(RGB_TO_SPECT_LAMBDAS,
                                              RGB_TO_SPECT_SAPMLES_COUNT),
            .values = std::span<const Float>(RGB_ILLUM_TO_SPECT_WHITE_VALUES,
                                             RGB_TO_SPECT_SAPMLES_COUNT),
        });

    static constexpr SampledSpectrum RGB_ILLUM_TO_SPECT_CYAN =
        SampledSpectrum::fromSortedSamples({
            .lambdas = std::span<const Float>(RGB_TO_SPECT_LAMBDAS,
                                              RGB_TO_SPECT_SAPMLES_COUNT),
            .values = std::span<const Float>(RGB_ILLUM_TO_SPECT_CYAN_VALUES,
                                             RGB_TO_SPECT_SAPMLES_COUNT),
        });

    static constexpr SampledSpectrum RGB_ILLUM_TO_SPECT_MAGENTA =
        SampledSpectrum::fromSortedSamples({
            .lambdas = std::span<const Float>(RGB_TO_SPECT_LAMBDAS,
                                              RGB_TO_SPECT_SAPMLES_COUNT),
            .values = std::span<const Float>(RGB_ILLUM_TO_SPECT_MAGENTA_VALUES,
                                             RGB_TO_SPECT_SAPMLES_COUNT),
        });

    static constexpr SampledSpectrum RGB_ILLUM_TO_SPECT_YELLOW =
        SampledSpectrum::fromSortedSamples({
            .lambdas = std::span<const Float>(RGB_TO_SPECT_LAMBDAS,
                                              RGB_TO_SPECT_SAPMLES_COUNT),
            .values = std::span<const Float>(RGB_ILLUM_TO_SPECT_YELLOW_VALUES,
                                             RGB_TO_SPECT_SAPMLES_COUNT),
        });

    static constexpr SampledSpectrum RGB_ILLUM_TO_SPECT_RED =
        SampledSpectrum::fromSortedSamples({
            .lambdas = std::span<const Float>(RGB_TO_SPECT_LAMBDAS,
                                              RGB_TO_SPECT_SAPMLES_COUNT),
            .values = std::span<const Float>(RGB_ILLUM_TO_SPECT_RED_VALUES,
                                             RGB_TO_SPECT_SAPMLES_COUNT),
        });

    static constexpr SampledSpectrum RGB_ILLUM_TO_SPECT_GREEN =
        SampledSpectrum::fromSortedSamples({
            .lambdas = std::span<const Float>(RGB_TO_SPECT_LAMBDAS,
                                              RGB_TO_SPECT_SAPMLES_COUNT),
            .values = std::span<const Float>(RGB_ILLUM_TO_SPECT_GREEN_VALUES,
                                             RGB_TO_SPECT_SAPMLES_COUNT),
        });

    static constexpr SampledSpectrum RGB_ILLUM_TO_SPECT_BLUE =
        SampledSpectrum::fromSortedSamples({
            .lambdas = std::span<const Float>(RGB_TO_SPECT_LAMBDAS,
                                              RGB_TO_SPECT_SAPMLES_COUNT),
            .values = std::span<const Float>(RGB_ILLUM_TO_SPECT_BLUE_VALUES,
                                             RGB_TO_SPECT_SAPMLES_COUNT),
        });

    SampledSpectrum
    SampledSpectrum::fromRGBReflectance(const std::array<Float, 3>& rgb) {
        SampledSpectrum result{};

        if (rgb[0] <= rgb[1] && rgb[0] <= rgb[2]) {
            result += rgb[0] * RGB_REFL_TO_SPECT_WHITE;

            if (rgb[1] <= rgb[2]) {
                result += (rgb[1] - rgb[0]) * RGB_REFL_TO_SPECT_CYAN;
                result += (rgb[2] - rgb[1]) * RGB_REFL_TO_SPECT_BLUE;
            }
            else {
                result += (rgb[2] - rgb[0]) * RGB_REFL_TO_SPECT_CYAN;
                result += (rgb[1] - rgb[2]) * RGB_REFL_TO_SPECT_GREEN;
            }
        }
        else if (rgb[1] <= rgb[0] && rgb[1] <= rgb[2]) {
            result += rgb[1] * RGB_REFL_TO_SPECT_WHITE;

            if (rgb[0] <= rgb[2]) {
                result += (rgb[0] - rgb[1]) * RGB_REFL_TO_SPECT_MAGENTA;
                result += (rgb[2] - rgb[0]) * RGB_REFL_TO_SPECT_BLUE;
            }
            else {
                result += (rgb[2] - rgb[1]) * RGB_REFL_TO_SPECT_MAGENTA;
                result += (rgb[0] - rgb[2]) * RGB_REFL_TO_SPECT_RED;
            }
        }
        else {
            result += rgb[2] * RGB_REFL_TO_SPECT_WHITE;

            if (rgb[0] <= rgb[1]) {
                result += (rgb[0] - rgb[2]) * RGB_REFL_TO_SPECT_YELLOW;
                result += (rgb[1] - rgb[0]) * RGB_REFL_TO_SPECT_GREEN;
            }
            else {
                result += (rgb[1] - rgb[2]) * RGB_REFL_TO_SPECT_YELLOW;
                result += (rgb[0] - rgb[1]) * RGB_REFL_TO_SPECT_RED;
            }
        }
    ...
    SampledSpectrum
    SampledSpectrum::fromRGBIlluminant(const std::array<Float, 3>& rgb) {
        SampledSpectrum result{};

        if (rgb[0] <= rgb[1] && rgb[0] <= rgb[2]) {
            result += rgb[0] * RGB_ILLUM_TO_SPECT_WHITE;

            if (rgb[1] <= rgb[2]) {
                result += (rgb[1] - rgb[0]) * RGB_ILLUM_TO_SPECT_CYAN;
                result += (rgb[2] - rgb[1]) * RGB_ILLUM_TO_SPECT_BLUE;
            }
            else {
                result += (rgb[2] - rgb[0]) * RGB_ILLUM_TO_SPECT_CYAN;
                result += (rgb[1] - rgb[2]) * RGB_ILLUM_TO_SPECT_GREEN;
            }
        }
        else if (rgb[1] <= rgb[0] && rgb[1] <= rgb[2]) {
            result += rgb[1] * RGB_ILLUM_TO_SPECT_WHITE;

            if (rgb[0] <= rgb[2]) {
                result += (rgb[0] - rgb[1]) * RGB_ILLUM_TO_SPECT_MAGENTA;
                result += (rgb[2] - rgb[0]) * RGB_ILLUM_TO_SPECT_BLUE;
            }
            else {
                result += (rgb[2] - rgb[1]) * RGB_ILLUM_TO_SPECT_MAGENTA;
                result += (rgb[0] - rgb[2]) * RGB_ILLUM_TO_SPECT_RED;
            }
        }
        else {
            result += rgb[2] * RGB_ILLUM_TO_SPECT_WHITE;

            if (rgb[0] <= rgb[1]) {
                result += (rgb[0] - rgb[2]) * RGB_ILLUM_TO_SPECT_YELLOW;
                result += (rgb[1] - rgb[0]) * RGB_ILLUM_TO_SPECT_GREEN;
            }
            else {
                result += (rgb[1] - rgb[2]) * RGB_ILLUM_TO_SPECT_YELLOW;
                result += (rgb[0] - rgb[1]) * RGB_ILLUM_TO_SPECT_RED;
            }
        }
