...
namespace idragnev::pbrt {
    ...
    std::optional<HitRecord> Sphere::intersect(const Ray& rayInWorldSpace,
                                               const bool) const {
    ...
    R Sphere::intersectImpl(const Ray& rayInWorldSpace,
                            F failure,
                            S success) const {
    ...
    Sphere::findIntersectionParams(const Ray& ray,
                                   const Vector3f& oErr,
        ...
        const auto c =
            ox * ox + oy * oy + oz * oz - EFloat(radius) * EFloat(radius);
    ...
    HitRecord Sphere::makeHitRecord(const Ray& ray,
                                    const Point3f& hitPoint,
                                    const EFloat& tShapeHit,
        ...
        const Vector3f dpdv =
            (thetaMax - thetaMin) * Vector3f(hitPoint.z * cosPhi,
                                             hitPoint.z * sinPhi,
                                             -radius * std::sin(theta));

        const Vector3f d2Pduu =
            -phiMax * phiMax * Vector3f(hitPoint.x, hitPoint.y, 0);
        const Vector3f d2Pduv = (thetaMax - thetaMin) * hitPoint.z * phiMax *
                                Vector3f(-sinPhi, cosPhi, 0.f);
        const Vector3f d2Pdvv = -(thetaMax - thetaMin) * (thetaMax - thetaMin) *
                                Vector3f(hitPoint.x, hitPoint.y, hitPoint.z);
        ...
        const Normal3f dndu = Normal3f((f * F - e * G) * invEGF2 * dpdu +
                                       (e * F - f * E) * invEGF2 * dpdv);
        const Normal3f dndv = Normal3f((g * F - f * G) * invEGF2 * dpdu +
