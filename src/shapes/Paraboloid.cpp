...
namespace idragnev::pbrt {
    Paraboloid::Paraboloid(const Transformation& objectToWorld,
        ...
        , phiMax{ 
            toRadians(clamp(phiMax, 0.f, 360.f)) 
          } 
    {
    }
    ...
    std::optional<HitRecord> Paraboloid::intersect(const Ray& rayInWorldSpace, const bool) const {
    ...
    R Paraboloid::intersectImpl(const Ray& rayInWorldSpace, F failure, S success) const {
    ...
    std::optional<QuadraticRoots> Paraboloid::findIntersectionParams(const Ray& ray, const Vector3f& oErr, const Vector3f& dErr) const {
    ...
    HitRecord Paraboloid::makeHitRecord(const RayWithErrorBound& rayWithErrBound, const Point3f& hitPoint, const EFloat& t, const Float phi) const {
        const auto dpdu = Vector3f(-phiMax * hitPoint.y, phiMax * hitPoint.x, 0.f);
        const auto dpdv = (zMax - zMin) * Vector3f(hitPoint.x / (2.f * hitPoint.z),
            hitPoint.y / (2.f * hitPoint.z),
            1.f);

        const auto d2Pduu = -phiMax * phiMax * Vector3f(hitPoint.x, hitPoint.y, 0.f);
        const auto d2Pduv = (zMax - zMin) * phiMax * Vector3f(-hitPoint.y / (2.f * hitPoint.z),
                                                               hitPoint.x / (2 * hitPoint.z),
                                                               0.f);
        const auto d2Pdvv = -(zMax - zMin) * (zMax - zMin) * Vector3f(hitPoint.x / (4 * hitPoint.z * hitPoint.z),
    ...
    Vector3f Paraboloid::computeHitPointError(const RayWithErrorBound& rayWithErrBound, const EFloat& t) const {
        const auto& [ray, oErr, dErr] = rayWithErrBound;
